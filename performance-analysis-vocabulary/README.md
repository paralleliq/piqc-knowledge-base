...
A workload is compute bound when the primary limitation is the rate at which the processor can execute instructions.

In this regime, the execution units (ALUs, FPUs, tensor cores, etc.) are fully utilized.
...
Higher IPC usually means the processor pipeline is being efficiently utilized.
...
A pipeline stall occurs when the processor cannot execute the next instruction due to missing data or resources.
...
Modern processors attempt to predict the outcome of branch instructions.
...
Compute-bound GPU workloads typically show:
...
A workload is memory bound when performance is limited by memory bandwidth or memory access patterns.

In this regime, processors frequently wait for data from the memory hierarchy.
...
This results in fast access.

Cache Miss

A cache miss occurs when the requested data is not present in the current cache level.

The processor must fetch it from the next level in the hierarchy.
...
Memory latency is the time required to retrieve data from memory.
...
Memory bandwidth refers to the maximum rate at which memory can supply data.
...
Memory pressure occurs when an application generates a high volume of memory accesses that stress the memory subsystem.
...
On GPUs, memory accesses are most efficient when threads access contiguous memory locations.

This allows the hardware to combine requests into fewer memory transactions.
...
Even if bandwidth is sufficient, high-latency operations can slow execution.
...
False sharing occurs when multiple cores modify variables located within the same cache line.

The cache line repeatedly moves between cores, causing excessive cache coherence traffic.

NUMA Remote Access

In multi-socket systems, accessing memory attached to another CPU socket increases latency.
...
In distributed or multi-device systems, communication delays can limit performance.
...
A bottleneck is the system resource that limits overall performance.
